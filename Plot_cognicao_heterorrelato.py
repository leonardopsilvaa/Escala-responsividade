...
def cognicao_plot_heterorrelato():
    # Conectar ao workbook e à planilha ativa
    wb = xw.Book.caller()
    sheet = wb.sheets[0]

    # Ler os valores de norma
    Norma_autorrelato = sheet.range('D10').value
    Norma_Heterorrelato = sheet.range('F10').value
    
    # Ler os valores de norma
    Dados_brutos_autorrelato = sheet.range('C10').value
    Dados_brutos_Heterorrelato = sheet.range('E10').value
...
#    # Plotar a curva normal
    ...
    nome_arquivo_png = 'cognicao.png'
    plt.savefig(nome_arquivo_png, bbox_inches='tight')
    plt.show()

    sheet.pictures.add(nome_arquivo_png, name='cognicao', update=True, top=sheet.range('T75').top, left=sheet.range('T75').left, height=400, width=700)
    time.sleep(1)
