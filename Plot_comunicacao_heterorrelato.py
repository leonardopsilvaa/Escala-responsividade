...
def comunicacao_plot_heterorrelato():
    # Conectar ao workbook e à planilha ativa
    wb = xw.Book.caller()
    sheet = wb.sheets[0]

    # Ler os valores de norma
    Norma_autorrelato = sheet.range('D11').value
    Norma_Heterorrelato = sheet.range('F11').value
    
    # Ler os valores de norma
    Dados_brutos_autorrelato = sheet.range('C11').value
    Dados_brutos_Heterorrelato = sheet.range('E11').value
...
#    # Plotar a curva normal
    ...
    nome_arquivo_png = 'comunicacao.png'
    plt.savefig(nome_arquivo_png, bbox_inches='tight')
    plt.show()

    sheet.pictures.add(nome_arquivo_png, name='comunicacao', update=True, top=sheet.range('T105').top, left=sheet.range('T105').left, height=400, width=700)
    # Remover o arquivo temporário
    os.remove(nome_arquivo_png)

if __name__ == "__main__":
    xw.Book("SRS-2.xlsm").set_mock_caller()
    comunicacao_plot_heterorrelato()
