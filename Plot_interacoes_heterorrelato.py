...
def interacoes_plot_heterorrelato():
    # Conectar ao workbook e à planilha ativa
    wb = xw.Book.caller()
    sheet = wb.sheets[0]

    # Ler os valores de norma
    Norma_autorrelato = sheet.range('D14').value
    Norma_Heterorrelato = sheet.range('F14').value
    
    # Ler os valores de norma
    Dados_brutos_autorrelato = sheet.range('C14').value
    Dados_brutos_Heterorrelato = sheet.range('E14').value
...
#    # Plotar a curva normal
    ...
    sheet.pictures.add(nome_arquivo_png, name='interacoes', update=True, top=sheet.range('T195').top, left=sheet.range('T195').left, height=400, width=700)
    #Remover o arquivo temporário
    os.remove(nome_arquivo_png)

if __name__ == "__main__":
    xw.Book("SRS-2.xlsm").set_mock_caller()
    interacoes_plot_heterorrelato()
