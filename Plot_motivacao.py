...
def motivacao_plot():
    ...
    norm_score = sheet.range('D12').value
    raw_score = sheet.range('C12').value
...
#    ax.fill_between(x, y, color='lightcyan', alpha=0.3)
    ...
    plt.savefig('motivacao.png', bbox_inches='tight')
    plt.show()


    sheet.pictures.add('motivacao.png', name='motivacao', update=True, top=sheet.range('T140').top, left=sheet.range('T140').left, height=400, width=700)
