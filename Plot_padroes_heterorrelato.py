...
def padroes_plot_heterorrelato():
    # Conectar ao workbook e à planilha ativa
    wb = xw.Book.caller()
    sheet = wb.sheets[0]

    # Ler os valores de norma
    Norma_autorrelato = sheet.range('D13').value
    Norma_Heterorrelato = sheet.range('F13').value
    
    # Ler os valores de norma
    Dados_brutos_autorrelato = sheet.range('C13').value
    Dados_brutos_Heterorrelato = sheet.range('E13').value
...
#    # Plotar a curva normal
    ...
    nome_arquivo_png = 'padroes.png'
    plt.savefig(nome_arquivo_png, bbox_inches='tight')
    plt.show()


    sheet.pictures.add(nome_arquivo_png, name='padroes', update=True, top=sheet.range('T165').top, left=sheet.range('T165').left, height=400, width=700)
    # Remover o arquivo temporário
    os.remove(nome_arquivo_png)

if __name__ == "__main__":
    xw.Book("SRS-2.xlsm").set_mock_caller()
    padroes_plot_heterorrelato()
