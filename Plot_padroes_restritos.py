...
def padroes_plot():
    ...
    norm_score = sheet.range('D13').value
    raw_score = sheet.range('C13').value
...
#    ax.fill_between(x, y, color='lightcyan', alpha=0.3)
    ...
    plt.savefig('padroes_restritos.png', bbox_inches='tight')
    plt.show()


    sheet.pictures.add('padroes_restritos.png', name='padroes_restritos', update=True, top=sheet.range('T170').top, left=sheet.range('T170').left, height=400, width=700)
