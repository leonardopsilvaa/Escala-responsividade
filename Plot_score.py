...
def score_plot():
    ...
    norm_score = sheet.range('D15').value
    raw_score = sheet.range('C15').value
...
#    ax.fill_between(x, y, color='lightcyan', alpha=0.3)
    ...
    sheet.pictures.add(nome_arquivo_png, name='score', update=True, top=sheet.range('T230').top, left=sheet.range('T230').left, height=400, width=700)
