...
def score_plot_heterorrelato():
    # Conectar ao workbook e à planilha ativa
    wb = xw.Book.caller()
    sheet = wb.sheets[0]

    # Ler os valores de norma
    Norma_autorrelato = sheet.range('D15').value
    Norma_Heterorrelato = sheet.range('F15').value
    
    # Ler os valores de norma
    Dados_brutos_autorrelato = sheet.range('C15').value
    Dados_brutos_Heterorrelato = sheet.range('E15').value
...
#    # Plotar a curva normal
    ...
    nome_arquivo_png = 'score.png'
    plt.savefig(nome_arquivo_png, bbox_inches='tight')
    plt.show()


    sheet.pictures.add(nome_arquivo_png, name='score', update=True, top=sheet.range('T225').top, left=sheet.range('T225').left, height=400, width=700)
    #Remover o arquivo temporário
    os.remove(nome_arquivo_png)

if __name__ == "__main__":
    xw.Book("SRS-2.xlsm").set_mock_caller()
    score_plot_heterorrelato()
