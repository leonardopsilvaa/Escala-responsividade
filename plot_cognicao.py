...
def cognicao_plot():
    ...
    norm_score = sheet.range('D10').value
    raw_score = sheet.range('C10').value
...
#    ax.fill_between(x, y, color='lightcyan', alpha=0.3)
    ...
    plt.savefig('cognicao.png', bbox_inches='tight')
    plt.show()


    sheet.pictures.add('cognicao.png', name='cognicao', update=True, top=sheet.range('T75').top, left=sheet.range('T75').left, height=400, width=700)
