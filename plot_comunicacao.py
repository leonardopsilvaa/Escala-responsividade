...
def comunicacao_plot():
    # Conectar ao workbook e à planilha ativa
    wb = xw.Book.caller()
    sheet = wb.sheets[0]

    # Ler os valores de norma e dados brutos
    norm_score = sheet.range('D11').value
    raw_score = sheet.range('C11').value
...
#    ax.fill_between(x, y, color='lightcyan', alpha=0.3)
    ...
    plt.savefig('comunicacao.png', bbox_inches='tight')
    plt.show()


    sheet.pictures.add('comunicacao.png', name='comunicacao', update=True, top=sheet.range('T110').top, left=sheet.range('T110').left, height=400, width=700)
