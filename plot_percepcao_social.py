...
def create_distribution_plot():
    ...
    norm_score = sheet.range('D9').value
    raw_score = sheet.range('C9').value
...
#    ax.fill_between(x, y, color='lightcyan', alpha=0.3)
    ...
    ax.plot(norm_score, 0, 'ro')  # Ponto vermelho na base do gráfico
    ...
    nome_arquivo_png = 'distribution_graph.png'
    plt.savefig(nome_arquivo_png, bbox_inches='tight')
    plt.show()


    sheet.pictures.add(nome_arquivo_png, name='distribution_graph', update=True, top=sheet.range('T45').top, left=sheet.range('T45').left, height=400, width=700)
